plot.catdes <- function(x,show= "all",level=0.01,sort=NULL,col.upper="indianred2",col.lower="royalblue1",numchar=10,barplot=FALSE,...){
  #Check the arguments
  if( !attr(x,"class")[1]=="catdes") {stop("'x' must be of type catdes")}
  if (show!="all" && show!="quanti" && show!="quali"){stop("Invalid value of show")}
...
if (!barplot){
  ...
  for (j in 1:(dim2)){ 
    ...
    ytop =1 - (1/(dim1 + 1))
    center <- c(mean(c(xleft,xright)),mean(c(ytop,ybottom)))
    text(center[1],1 - (1/(dim1 + 1)),columns[j],cex=0.4,srt=45,adj=c(0,0))
  }
