...
###############################################################
#calcul les coefficients de dilatation pour les partiels 
###############################################################
hdil <- function(H) {
    nbnivh <- length(H)
    dil <- H
...
###############################################################
#décrit la hiérarchie en termes de nombres de variables
###############################################################
#htabdes ne marche que pour une hiérarchie
...
################# Main program
    ...
    for (h in 1:nbnivo) {
      ...
      part1[[h]] <- part2
      if (!is.null(ind.quali)) {
        for (k in 1:length(ind.quali)) part2.quali[k,,] <- apply(part2[X[,ind.quali[k]]>0,,], c(2,3),mean)
    ...
    if (!is.null(ind.quali)) {
      ...
      for (k in 1:length(ind.quali)) aux[k,] <- apply(res.afmh$ind$coord[X[,ind.quali[k]]>0,], 2,mean)
