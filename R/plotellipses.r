plotellipses <- function (model, keepvar = "all", axis = c(1, 2), means=TRUE, level = 0.95, magnify = 2, cex = 0.5, pch = 20, pch.means=15, type = c("g","p"), keepnames = TRUE, namescat = NULL, ...) 
{
    monpanel <- function(x, y, level, means, nommod, magnify = magnify, pchmeans=pchmeans, ...) {
        panel.xyplot(x, y, ...)
        panel.superpose2(x, y, level = level, means=means, nommod = nommod,
        pchmeans=pchmeans, magnify = magnify,
    ...
    monpanel.ellipse <- function(x, y, col.line, lty, lwd, subscripts, 
        ...
        lpoints(cdg[1], cdg[2],pch=pchmeans,col = col.line, cex = cex * magnify)
    }
    ...
    if (class(model)[1]=="PCA") {
      if (all(names(model$call)!="quali.sup")) return(NULL) else {
    ...
    if (class(model)[1]=="MCA") {
      ...
      if (is.character(keepvar)) {
        if (length(keepvar) == 1) {
          ...
          if (choix == 3) 
            eliminer <- (1:nbevartot)[-model$call$quali.sup]
          if (choix == 1) {
    ...
    if (!is.logical(keepnames)) {
        ...
        else nomvartrimmees <- nomvargardees[!(nomvargardees %in% 
            keepnames)]
    }
    else {
        if (length(keepnames) == 1) {
            ...
            if (length(keepnames) == length(nomtot)) 
                nomvartrimmees <- nomtot[(!keepnames) & ((1:nbevartot) != 
                  eliminer)]
            else return(NULL)
        }
      }
     ...
     if (!is.null(eliminer)) 
      donnees <- model$call$X[, -eliminer,drop=FALSE] else  donnees <- model$call$X
    nbevar <- ncol(donnees)
    nindiv <- nrow(donnees)
    don <- apply(model$ind$coord[, axis], 2, FUN = function(x, 
        k) rep(x, k), k = nbevar)
    rownames(don) <- NULL
    colnames(don) <- c("x", "y")
    ...
    if (!is.null(nomvartrimmees) & (is.null(namescat))) {
        ...
        selecti <- as.vector(mapply(seq, (kept - 1) * nindiv + 
            1, (kept) * nindiv))
        nommod[selecti] <- substr(nommod[selecti], nchar(nomvar[selecti]) + 
            2, nchar(nommod[selecti]))
    }
    don <- cbind.data.frame(don, var = nomvar, modalite = factor(modalite2))
    xyplot(y ~ x | var, data = don, groups = modalite, panel = monpanel, 
        level = level, means=means, magnify = magnify, cex = cex, pch = pch,
        pchmeans = pch.means, 
        nommod = nommod, type = type, xlab =paste("Dim ", axis[1]," (",round(model$eig[axis[1],2],2),"%)",sep=""),
         ylab = paste("Dim ", axis[2]," (",round(model$eig[axis[2],2],2),"%)",sep=""))
}
