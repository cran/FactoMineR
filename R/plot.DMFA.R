...
if (choix=="quali"){
  ...
  xlim = 1.1*c(min(res.dmfa$quali.sup$coord[,axes[1]]),max(res.dmfa$quali.sup$coord[,axes[1]]))
  ylim = 1.1*c(min(res.dmfa$quali.sup$coord[,axes[2]]),max(res.dmfa$quali.sup$coord[,axes[2]]))
  ...
  abline(v = 0, lty = 2)
  abline(h = 0, lty = 2)
  for (i in 1:sum(res.dmfa$call$quali.sup$modalite[2:(1+nb.quali)])) {
...
## Ajout des variables partielles
  ...
  for (j in 1:ng) {
    ...
    prefpls(cbind(auxil,Xc[[j]][rownames(auxil),]),title=paste("Biplot between axes ",axes[1]," and ",axes[2]," for group ",lev[j],sep=""))
  }
} 
if (choix=="group"){
