AovSum <- function (formula, data, na.action = na.omit, ...){
  ...
  colnames(test.F)[2] = "ddl"
  colnames(test.F)[3] = "MS"
  test.T = summary.lm(modele)$coef
  cov.mat = vcov(modele)
  facteurs = rownames(attr(modele$terms,"factors"))[-1]
  interact = NULL
##  if (length(strsplit(as.character(formula)[3],":")[[1]])>1) interact =  colnames(attr(modele$terms,"factors"))[-(1:length(facteurs))]
  if (length(colnames(attr(modele$terms,"factors")))> length(facteurs)) interact =  colnames(attr(modele$terms,"factors"))[-(1:length(facteurs))]

  #niveau = modele$xlevels
  niveau=list()
  for (i in 1:length(facteurs)){
    if (is.factor(don[,facteurs[i]])) niveau[[i]] = paste(facteurs[i],levels(don[,facteurs[i]]),sep=" - ")
    else niveau[[i]] = facteurs[i]
  }

  res = test.T[c(1,1),]  ## just to have a matrix
  iinit=2
  for (i in 1:length(facteurs)) {
