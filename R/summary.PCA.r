summary.PCA <- function(object,nb.dec=3,nbelements=10,nbind=nbelements,ncp=3,align.names=TRUE,file="",...){
...
print3 <- function(obj,file="",ncp,width.row=0,nbelements=nbelements){
  ...
  res <- object
  if (!inherits(res, "PCA")) stop("non convenient object")
  cat(paste("\nCall:\n"),file=file)
...
if (nbind>0){
   ...
   if (nrow(res$ind$coord) > nbind) cat(paste(" (the ",nbind," first)",sep=""),file=file,append=TRUE)
   cat("\n",file=file,append=TRUE)
  ...
  if (!is.null(res$ind.sup)){
   ...
   cat("\nVariables",file=file,append=TRUE)
