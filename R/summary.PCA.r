summary.PCA <- function(object,nb.dec=3,nbelements=10,ncp=3,align.names=TRUE,file="",...){
...
print3 <- function(obj,file="",ncp,width.row=0,nbelements=nbelements){
  ...
  if (align.names==TRUE){
   ...
   if (nrow(res$ind$coord) > nbelements) cat(paste(" (the ",nbelements," first)",sep=""),file=file,append=TRUE)
   cat("\n",file=file,append=TRUE)
