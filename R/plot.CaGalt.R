plot.CaGalt<-function(x, axes = c(1, 2), choix = c("ind", "freq", "quali.var", "quanti.var"), ellipse = FALSE, contr.ellipse = 3, xlim = NULL, ylim = NULL, col.ind = "black", col.freq = "red", col.quali = "blue", col.quanti = "blue", label = TRUE, lim.cos2.var = 0, title = NULL, palette = NULL, autoLab = c("auto", "yes", "no"), new.plot = FALSE, select = NULL, unselect = 0.7, shadowtext = FALSE, ...){
	res.cagalt <- x
	...
	if (choix == "freq") {
		...
		if (!shadowtext) points(coo[, 1], y = coo[, 2], pch = ipch, col = coll, ...)
		if (ellipse) {
			if(is.null(res.cagalt$ellip)) stop("The coordinates of the ellipses to construct confidence ellipses were not computed by CaGalt")
			...
			for(i in 1:length(sel.freq)){
				...
				lines(coord.ellip$res[,2],coord.ellip$res[,3],lty=2,lwd=2,col="darkgrey")
			}
		}
	}
	if (choix == "quanti.var") {
