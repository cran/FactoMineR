CaGalt<-function(Y,X,type="s",conf.ellip=FALSE,nb.ellip=100,level.ventil=0,sx=NULL,graph=TRUE,axes=c(1,2)){
	...
	if(type!="n"){
		...
		X<-sweep(tab.disjonctif(X),2,apply(tab.disjonctif(X),2,mean.p,PI.),"-")
	}
	...
	res<-list(eig=diag.L$eig,ind=res.ind,freq=res.freq)
	...
	class(res)<-c("CaGalt","list")		
	if (graph){	
		plot.CaGalt(res,choix="ind",axes=axes)
		plot.CaGalt(res,choix="freq",axes=axes,ellipse=!is.null(res$ellip),new.plot=TRUE)
		if (type!="n") plot.CaGalt(res,choix="quanti.var",axes=axes,ellipse=!is.null(res$ellip),new.plot=TRUE)
		else plot.CaGalt(res,choix="quali.var",axes=axes,ellipse=!is.null(res$ellip),new.plot=TRUE)
	}
	return(res)
}